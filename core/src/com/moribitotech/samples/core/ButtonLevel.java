...
public class ButtonLevel extends AbstractButton {
	...
	private void drawStars(Batch batch) {
		if (textureStarHolder != null && textureStar != null) {
			...
			float activePosXStart = (getX() + getWidth() / 2)
					- ((starSizeWidth * numberOfTotalStars) / 2);
			...
			for (int i = 0; i < numberOfTotalStars; i++) {
				...
				batch.draw(textureStarHolder, currentPosX + starPosXStart,
						activePoxYStart + starPosYStart, starSizeWidth,
						starSizeHeight);
			}
			...
			for (int j = 0; j < numberOfEarnedStars; j++) {
				...
				batch.draw(textureStar, currentPosX + starPosXStart,
						activePoxYStart + starPosYStart, starSizeWidth,
						starSizeHeight);
			}
		}
	...
	private void drawLevelNumber(Batch batch) {
		...
		if (levelNumber < 10) {
			bitMapFont.draw(batch, "" + levelNumber, getX() + getWidth()
					/ singePositionArranger, getY() + getHeight() / 1.4f);
		} else {
			bitMapFont.draw(batch, "" + levelNumber, getX() + getWidth()
					/ doublePositionArranger, getY() + getHeight() / 1.4f);
		}
	}
	 ...
	 * <p>
	 * EXAMPLE<br>
	 * There are 5 stars maximum can be achieved, and user earned 3 stars. It
	 * auto calculates the position and sizes of stars due to button sizes.
	 * However further adjustment can be done (optinal) with
	 * "setLevelStarSizeRatio", "setLevelStarPosXStart" and
	 * "setLevelStarPosYStart". !!starHolderTexture and starTexture should be in
	 * same sizes!!
	 * 
	 * @param starHolderTexture
	 *            the holder texture for star or other achievement object.
	 * @param starTexture
	 *            the texture for star or other achievement object.
	 * @param numberOfTotalStars
	 *            number of total stars
	 * @param numberOfEarnedStars
	 *            number of earned/achieved stars
	 * */
	public void setLevelStars(TextureRegion starHolderTexture,
			TextureRegion starTexture, int numberOfTotalStars,
			int numberOfEarnedStars) {
	...
	/**
	 * Set level stars X position. This is being calculated automaticly and
	 * centered. Do not suggest to play width.
	 * */
	...
	/**
	 * Set level stars Y position. This can be used to lift all stars (holders
	 * and stars) up or down for adjusments
	 * */
	...
	/**
	 * Set level stars size ratio. This makes stars smaller or bigger (Default
	 * is 5). Bigger number makes stars/holders bigger and vice-versa.
	 * */
	...
	/**
	 * Set number of earned achievement object. This number cannot be
	 * higher/lower than numberOfTotalStars, it auto overrides if is less or
	 * more than numberOfTotalStars
	 * */
